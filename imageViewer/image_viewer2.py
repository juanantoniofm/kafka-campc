...
# Modified by: juanantoniofm
# ----------------------------------------

"""
quick hack to display pics in a folder.
It only works with jpged, and is harcoded to use my dev folder.
If you wanna be able to choose the folder, just go to the method 
onOpenDirectory, and replace that one with her sister function *_DIALOG
a few lines below.
glob doesn't seem to work for some reason, so I also created another fun
...
# Fix for any wx >= 2.8 (my case) 
# only in app's startup  module  from wx.lib.pubsub import pub as Publisher
# in all modules that use pubsub 

#from wx.lib.pubsub import Publisher # deprecated
...
def pictureList(picPath = None):
    """
    dirty hack to get the list of pics
    """
    files = os.listdir(picPath)
    print "DEBUG","Files", files
    def is_it_a_pic(filename):
        if "jpeg" in filename[-4:] or "jpg" in filename[-4:]:
            return os.path.join(picPath, filename)
        else:
            return None

    image_list = map(is_it_a_pic, files)
    print "DEBUG","image list", image_list
    return image_list
...
class ViewerFrame(wx.Frame):
    ...
    def onOpenDirectory(self,event):
        self.folderPath = "/home/jmorales/workspace/src/kafka.campc/captures"
        picPaths = pictureList(self.folderPath)
        Publisher.sendMessage("update images", picPaths)



    def onOpenDirectory_DIALOG(self, event):
        """
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            print "DEBUG:","folderPath", self.folderPath
            picPaths = pictureList(self.folderPath)
            #picPaths = glob.glob(self.folderPath + r"\*.jpg") #SHIT
            print "DEBUG:","picPaths",picPaths
        Publisher.sendMessage("update images", picPaths)
